...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        lines.append(
            f"{trunc(r['prompt']):<40} | {trunc(r['gpt4o_answer']):<40} | "
            f"{trunc(r['mini_answer']):<40} | {r['gpt4o_time']:<15.3f} | "
            f"{r['mini_time']:<15.3f}"
        )
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    # #2. Test chat_with_system_prompt (Thêm print tại đây)
    # print("\n=== Test Chat with System Prompt ===")
    # reply, latency = chat_with_system_prompt(
    #     system_prompt="Bạn là kỹ sư phần mềm senior, trả lời chính xác, có ví dụ code khi phù hợp.",
